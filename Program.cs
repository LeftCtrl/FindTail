...
namespace FindTail
{
  class Program
  {
    ...
    private static void FindTail()
    {
      ...
      //Сложность - O(n) - два прохода по данным в худшем случае, когда "петля" начинается в конце массива
      int tailStart = GetTailStart(testData, tailLength); 
      ShowLog("tail started from (answer): " + tailStart);

      //Т.к. все части алгоритма имеют сложность O(n), то и алгоритм имеет сложность O(n)
